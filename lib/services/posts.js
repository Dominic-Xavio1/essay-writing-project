...
export async function createPost(authorId, data) {
  ...
  if (initialStatus === 'pending') {
    await notifySuperusersOnNewPost(data.title, postId);
  }

  return getPostById(postId, authorId);
}

export async function moderatePost(postId, newStatus) {
  const { rows } = await query(
    `UPDATE posts SET status = $2 WHERE id = $1 RETURNING author_id, title`,
    [postId, newStatus]
  );
  if (!rows.length) return null;
  await notifyAuthorOnModeration(rows[0].author_id, rows[0].title, newStatus);
