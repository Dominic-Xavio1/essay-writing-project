...
export async function getUserStats(userId) {
  try {
    const { rows } = await query(
      `SELECT
         (SELECT COUNT(*)::int FROM posts WHERE author_id = $1 AND status IN ('approved', 'published')) AS posts,
         (SELECT COUNT(*)::int FROM likes l JOIN posts p ON l.post_id = p.id WHERE p.author_id = $1) AS likes,
         (SELECT COUNT(*)::int FROM follows WHERE following_id = $1) AS followers,
         (SELECT COUNT(*)::int FROM follows WHERE follower_id = $1) AS following,
         (SELECT COUNT(*)::int FROM post_reads pr JOIN posts p ON pr.post_id = p.id WHERE p.author_id = $1) AS total_reads,
         (SELECT COALESCE(AVG(pr.scroll_depth), 0)::float FROM post_reads pr JOIN posts p ON pr.post_id = p.id WHERE p.author_id = $1) AS avg_retention,
         (SELECT COALESCE(AVG(pr.read_duration), 0)::int FROM post_reads pr JOIN posts p ON pr.post_id = p.id WHERE p.author_id = $1) AS avg_duration`,
      [userId]
    );
    const s = rows[0];
    const avgDurationSeconds = Number(s?.avg_duration ?? 0);
    const mins = Math.floor(avgDurationSeconds / 60);
    const secs = avgDurationSeconds % 60;
    const formattedDuration = avgDurationSeconds > 0 ? `${mins}m ${secs}s` : '0m 0s';
    const retentionRate = Math.min(100, Math.round(Number(s?.avg_retention ?? 0)));
    ...
    return {
      ...
      totalReads: Number(s?.total_reads ?? 0),
