...
export default function DashboardPage() {
  ...
  const [activeTab, setActiveTab] = useState<'published' | 'drafts' | 'bookmarked' | 'analytics'>('published');
  ...
  const publishedPosts = userPosts.filter(
    (p) => (p as Post & { status?: string }).status !== 'draft'
  );
  const draftPosts = userPosts.filter(
    (p) => (p as Post & { status?: string }).status === 'draft'
  );
    ...
    <>
      ...
      <main className="min-h-screen bg-background pb-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
          ...
          {/* Sleek Interactive Analytics Cards with Sparklines */}
          ...
          <AnimatePresence mode="wait">
            ...
            {activeTab === 'bookmarked' && (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0 }}
                className="text-center py-16 px-6 bg-card border border-border rounded-2xl max-w-lg mx-auto"
              >
                ...
                <h3 className="font-serif text-2xl font-bold text-foreground mb-2">Bookmarked Essays</h3>
                <p className="text-muted-foreground text-sm leading-relaxed mb-6">
