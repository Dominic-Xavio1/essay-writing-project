...
function sizeCanvas(canvas, ctx, box) {
 ...
 */
function computeBurstBox(rect, startVelocity) {
...
function createAshSimulation(canvas, button, options) {
...
const EmojiBurstButton = React.forwardRef(
  (
    ...
    const startBurst = React.useCallback(() => {
      ...
      const loop = () => {
        const stillActive = paintSim(sim, button);
        if (stillActive) {
      ...
      <>
        <MotionButton
          ...
          onClick={(e) => {
            ...
            if (onReact) onReact();
            if (!preferReducedMotion) startBurst();
          }}
          {...props}
        >
          {children || label}
